...
class intrusive_list {
	...
	template <bool IsConst> class iter_base {
	...
	public:
		...
		operator pointer() const { return intrusive_list::ptr_from_node(it); }
...
public:
	...
	void push_back(T* node) { impl_insert(&sentinel, &(node->*NodePtr)); };
	void push_front(T* node) { impl_insert(sentinel.next, &(node->*NodePtr)); }
	...
	template <typename Cb> void clear(Cb fn)
	{
...
template <typename T> class intrusive_fwd_list {
...
public:
	...
	void pop_front_unsafe() { first_node = first_node->fwd_list_node.next; }
