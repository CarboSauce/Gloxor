...
[[gnu::used,
	gnu::section(".limine_requests_start")]] static volatile std::uint64_t
	limine_requests_start_marker[] = LIMINE_REQUESTS_START_MARKER;

[[gnu::used,
	gnu::section(".limine_requests_end")]] static volatile std::uint64_t
...
void limine_main()
{
	if (rsdp_request.response != nullptr)
		archInfo.acpiRsdp = (paddrT)rsdp_request.response->address;

	if (framebuffer_request.response != nullptr) {
		if (framebuffer_request.response->framebuffer_count >= 1) {
			auto fb = framebuffer_request.response->framebuffers[0];
			machineInfo.fbInfoEntry.begin = (paddrT)fb->address;
			...
			gx::term::init_term((color_t*)fb->address,
