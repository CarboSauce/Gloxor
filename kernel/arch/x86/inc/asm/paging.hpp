...
struct PageEntry
{
    ...
    u64 entry;
    void set(vaddr addr, PagingBits mask)
    {
...
struct alignas(0x1000) PageTable
{
    ...
    static_assert(
        lvl > 0 && lvl < 5,
        "Page table level must be between 1 and 4"
    );
    ...
    auto index(vaddr addr) const
    {
        return (addr >> shift) & 0x1ff;
    }
    const PageEntry<I>& entry(vaddr addr) const
    {
        return entries[index(addr)];
    }
    PageEntry<I>& entry(vaddr addr)
    {
        // :tf:
     ...
     */
    const auto* next(vaddr addr) const
    {
        static_assert(
