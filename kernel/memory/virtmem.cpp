...
{
	auto nvmap = glox::find_if(vmapList.begin(), vmapList.end(), [len](auto&& it) { return it.virt_base + it.len + len < VmapIter(&it).next()->virt_base; });
...
{
	auto res = glox::find_if(begin(vmapList), end(vmapList), [addr](auto&& it) { return it.virt_base == (vaddrT)addr; });
	if (res == end(vmapList))
		return;
	vmapList.erase(res);
