...
inline bool is_overlapping(
	uintptr base, uintptr back, uintptr obase, uintptr oback)
{
...
inline bool is_there_dup(uintptr base, size_t len)
{
	for (const auto& it : vmapList) {
		if (is_overlapping(
				it.phys_base, it.phys_base + it.len, base, base + len)) {
			return true;
		}
	}
	return false;
...
void vunmap_iomem(void* addr)
{
	auto res = glox::find_if(begin(vmapList), end(vmapList), [addr](auto&& it) {
		return it.virt_base == (vaddrT)addr;
	});
	if (res == end(vmapList))
		return;
	vmapList.erase(res);
...
VSpace kAddrSpace {};
inline auto to_mask(PagePrivileges p, PageCaching c) { return (u64)p | (u64)c; }
...
void gx::VSpace::make_current() { arch::vmem::set_context(ptable); }
